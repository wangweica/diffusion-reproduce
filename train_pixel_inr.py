...
volume_sequence = sio.loadmat("/home/Data/zhangxiao/datasets/OLIVES/selected_mats/test/02-043_OD_S2T.mat")['S2T']
# 改为拟合图像
volume_sequence = torch.from_numpy(volume_sequence).float()
...
inr_input_data = INRInputData(volume_sequence, volume_sequence.shape, B_gauss, max_val.item(), min_val.item())
# 初始化模型、损失函数和优化器
channel_list = []
    ...
    for epoch in range(num_epochs):
        for inputs, labels in tqdm(dataloader, desc=f'Epoch {epoch + 1}/{num_epochs}', unit='batch'):
            ...
            if step % save_interval == 0:
                ...
                evaluate(model, inr_input_data, use_wandb=is_wandb, device=device)
