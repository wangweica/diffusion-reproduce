...
# 定义INR模型
exp_name = f"TIF-encoder-ch64-[1,2,4,8]"
data_path = 'data'
...
unet_config = {'image_size': (128, 128), 'in_channels': 2,
...
if is_train:
    ...
    for epoch in range(num_epochs):
        ...
        for i in tqdm(timeIdx, desc=f'Epoch {epoch + 1}/{num_epochs}', unit='batch'):
            ...
            outputs = model(inputs, timesteps=torch.ones((1,)).to(device), coords=t)
            ...
            if step % save_interval == 0:
                print("输出最大值最小值:", outputs.max().item(), outputs.min().item())
                outputs = torch.clamp(outputs, 0, 1)
                predictions = postprocess_volume(outputs, min_val, max_val)
                gt = postprocess_volume(label, min_val, max_val)
                slice = predictions.squeeze()[42].detach()
                slice_gt = gt.squeeze()[42].detach()
